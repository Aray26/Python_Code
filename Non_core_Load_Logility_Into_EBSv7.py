...
#df = pd.read_excel('Lindsey Done With Adjustments Altered.xlsx', sheet_name='NONCORE')
#df = pd.read_excel('CaseyG-CIM.xlsx', sheet_name='CIM 2016')
df = pd.read_excel('non_core_new_logility_load4.xlsx', sheet_name='NONCORE')
...
target.write("M,NONCORE_FC,NONCORE_FC_DESC,Days,365,365,NJW,N\r\n") #Extra space bug - 
for x in range(1, length_of_columns):
      for i in range(0, length_of_rows):
            ...
            #target.write("D,NONCORE,AUG17,,"+str(row_value)+","+ str(date_value).upper()+",,"+ str(data_value)+",N\r\n")
