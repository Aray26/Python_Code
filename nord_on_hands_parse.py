...
#df = pd.read_excel('Just_last_receipt_week.xlsx', sheet_name='Sheet1')
df = pd.read_excel('input_raw.xlsm',  sheetname = 'Extract')
...
# Business Unit Column 1
business_unit_column = df[df.columns[0]].tolist()
print type(business_unit_column)

print business_unit_column

# Business Unit
for row_element in business_unit_column:
    element = []
    element = str(row_element).split(',')
    #print element[0]

# Location Block Column 2
location_column = df[df.columns[1]].tolist()
print type(location_column)

print location_column

for row_element in location_column:
    element = []
    element = str(row_element).split(',')
    print element[0]

# Department Block Column 3
department_column = df[df.columns[2]].tolist()
print type(department_column)

print department_column

for row_element in department_column:
    element = []
    element = str(row_element).split(',')
    print element[1]

#  VPN Block Column 4
vpn_column = df[df.columns[2]].tolist()
print type(vpn_column)

print vpn_column

for row_element in vpn_column:
    element = []
    element = str(row_element).strip()
    print element[0]

#  VPN Block Column 4
#vpn_column = df[df.columns[VPN]].tolist()
vpn_column = df['VPN']
print type(vpn_column)
...
for row_element in vpn_column:
    element = []
    print str(row_element).strip()

#  EOH $ Block Column 4
#vpn_column = df[df.columns[VPN]].tolist()
eoh_column = df['EOH $']
print type(eoh_column)
print eoh_column

for row_element in eoh_column:
    now_int = int(row_element)
    print str(now_int)
...
for row in df.iterrows():
    ...
    print "this is the type of my row" + str(type(row))  # print (row['Last Receipt Week']).split(',')
    print "the length of my row " + str(len(row))
'''
'''
    index, year_period_week = row
    print "year_period_week " + str(year_period_week)
    print "hhhhhhhhhhhhhhhhh"
    print "Year Period Week " + str(year_period_week)
    print "len of year_period_week " + str(len(year_period_week))
    print "type is Year_period_week" + str(type(year_period_week))
    listy = str(year_period_week).split(',')
    print "listy is " + str(listy)
    for element in listy:
        print "Element is " + str(element)
        print "listy[0] " + str(listy[0].split(' ')[-1]).strip()
        print "listy[1] " + str(listy[1]).strip()
        print "listy[2] " + str(listy[2]).strip()

        exit
        # year, period, week  =  year_period_week

        # print year
        # print period
        # print week_number
        # print data_row.split(',')
        # array_row_elements = individual_row
        # print(str(type(array_row_elements)))
